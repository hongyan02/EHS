...
import DangerSourceTable from "@/components/danger-source/danger-soure-table";
import DangerSourceActionBar from "@/components/danger-source/danger-source-action-bar";

/**
 * 危险源页面组件
 * @returns {JSX.Element} 危险源页面
 */
export default function DangerSourcePage() {
    // 设置页面标题
    useEffect(() => {
        document.title = "危险源管理 - EHS";
    }, []);
    ...
    const _handleDataChange = () => {
        // TODO: 这里可以触发React Query的invalidateQueries来刷新数据
        console.log("数据已更新，需要刷新表格");
    };

    return (
        <div className="min-h-screen w-full bg-gray-50 flex flex-col">
            <div className="w-full flex-1 flex flex-col p-3 sm:p-4 lg:p-6">
